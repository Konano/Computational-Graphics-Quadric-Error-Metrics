...
* @Last Modified time: 2019-06-29 14:42:44
*/
...
inline Pair getCost(int u, int v) {
    ...
    if (verV[u].size() != triV[u].size())
        deltaV *= (deltaV < 0 ? -1 : 1) * 10000;
    if (verV[v].size() != triV[v].size())
        deltaV *= (deltaV < 0 ? -1 : 1) * 10000;
...
inline bool reversal(int a, int b, Vec o) {
    Vec tmp;
    V[numV] = o;
...
inline void deleteV() {
    ...
    if (reversal(mn.u, mn.v, mn.bestV)) return;
    int newV = numV++;
    V[newV] = mn.bestV;
    ...
    traversal(set<int>, needUpdateV) {
        ...
        _traversal(set<int>, verV[*it])
            if (needUpdateV.find(*_it) == needUpdateV.end())
